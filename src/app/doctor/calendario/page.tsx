...
export default function CalendarioPage() {
  ...
  useEffect(() => {
    if (newApptDate && doctor) {
      ...
      setNewApptTime(null);
      const dayMod = doctor.modalityByDay?.[dayNames[newApptDate.getDay()]];
      if (dayMod === 'online') setNewApptModality('online');
      else if (dayMod === 'in-person' || dayMod === 'presencial') setNewApptModality('in-person');
    }
  ...
  const handleConfirmPayment = async (appointmentId: string, method: string) => {
    ...
    try {
      await appointmentsApi.confirmPayment(appointmentId, method);
      setMessage({ type: 'success', text: 'Pago confirmado exitosamente' });
      loadAppointments();
    } catch (err: unknown) {
      setMessage({ type: 'error', text: err instanceof Error ? err.message : 'Error al confirmar pago' });
    } finally {
              ...
              <>
                ...
                {newApptPatient && newApptTime && (
                  <div className="bg-salmon-50 rounded-lg p-4 mb-4">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{newApptModality === 'online' ? 'En línea (Google Meet)' : 'Presencial'}</p>
                  </div>
                )}
                      ...
                      <div key={apt.id} className="p-3 bg-gray-50 rounded-lg">
                        ...
                        {apt.paymentStatus !== 'paid' && (
                          <div className="mt-2 flex gap-1.5">
                            <button
                              onClick={() => handleConfirmPayment(apt.id, 'transfer')}
                              disabled={confirmingId === apt.id}
                              className="px-2.5 py-1 bg-green-500 text-white text-xs font-medium rounded-lg hover:bg-green-600 disabled:opacity-50 transition-colors"
                            >
                              {confirmingId === apt.id ? '...' : 'Transferencia'}
                            </button>
                            <button
                              onClick={() => handleConfirmPayment(apt.id, 'cash')}
                              disabled={confirmingId === apt.id}
                              className="px-2.5 py-1 bg-green-500 text-white text-xs font-medium rounded-lg hover:bg-green-600 disabled:opacity-50 transition-colors"
                            >
                              {confirmingId === apt.id ? '...' : 'Efectivo'}
                            </button>
                          </div>
                        )}
                      </div>
